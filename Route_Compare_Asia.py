...
with open(directory+'Device_list_Asia.txt') as f:
  list_cores = [line.split() for line in f]

for switch in list_cores:
  if switch[2] == 'A':
	os.system("bin/./clogin -x "+directory+"arista "+switch[0]+" >> "+ directory + today + "/" +switch[1]+".txt")
  else:
	os.system("bin/./clogin -x "+directory+"cisco "+switch[0]+" >> "+ directory + today + "/" +switch[1]+".txt")
#  file1 = directory + today + "/" +switch[1]+".txt"
#  file2 = directory + yesterday + "/" +switch[1]+".txt"
#  diff = difflib.unified_diff(open(file2).readlines(),open(file1).readlines())
#  lines = list(diff)[2:]
#  added = [line[1:] for line in lines if line[0] == '+']
#  removed = [line[1:] for line in lines if line[0] == '-']
#  with open (directory+'Diff_' + today + '.txt', 'a') as changefromyesterday:
...
#os.system ("cat "+directory+"Diff_"+today+".txt | grep -v login")
